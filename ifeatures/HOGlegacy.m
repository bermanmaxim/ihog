function features = HOGlecacy(input_image, window)
%DENSE_HOG compute dense 36=channel legacy HOG features from an image.
%
%    features = featuresM(input_image)
%    features = featuresM(input_image, window)
%
% Maxim 36=channel legacy HOG implementation.
%
...
%  t_features = 0.2357 * cat(3, t1, t2, t3, t4);
  % Format the result.
  features = cat(3, h1, h2, h3, h4);
  features = features(window+1:window:end-window,window+1:window:end-window,:); %discretization
end
